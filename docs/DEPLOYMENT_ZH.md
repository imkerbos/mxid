...
| `MXID_BOOTSTRAP_ADMIN_PASSWORD` | 种子 `admin` 账户的初始口令。迁移 000009 给该账户设的口令明文写在那个迁移文件里、在公开仓库中。设了此变量,启动时就替换掉它。**不设则账户仍可登录,但欠一次改密**,且每次启动都会打醒目告警 | 强随机;无论哪种情况首次登录都会强制改密 |
...
| `MXID_BOOTSTRAP_ADMIN_PASSWORD` | **release 建议设置** | — | 种子 `admin` 账户的初始口令。迁移 000009 给该账户设的口令,明文就写在那个迁移文件的第一行、在公开仓库里。设了此变量,账户就用你给的口令。不设则账户**仍可登录**,但欠一次改密,且每次启动打醒目告警——所以全新部署要么设这个变量,要么登录后立刻改密。早前的版本是**直接锁掉账户**,那让全新部署(`admin` 是唯一账户)变成没人能登录。两种情况下首次登录都仍会强制改密,因为这个值通常躺在部署清单里,能看到它的人比该知道口令的人多。Helm 对应 `secrets.bootstrapAdminPassword` |
...
- [ ] 首次启动前已设 `MXID_BOOTSTRAP_ADMIN_PASSWORD`,或启动后立刻改掉了 `admin` 的口令。在此之前该账户接受的是本仓库里公开的口令。首次登录的强制改密已完成,MFA 已绑。
