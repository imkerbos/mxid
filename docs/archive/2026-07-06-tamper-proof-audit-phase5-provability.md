...
- **`VerifyExport` proves the entry-hash chain, not payload binding**: since the HMAC key is never exported, `VerifyExport` recomputes the Merkle root over the exported `EntryHash` values but cannot recompute `EntryHash` from `Payload` — a doctored `Payload` sitting next to its original, correct `EntryHash` passes offline verification. Payload↔hash integrity is only checked operator-side by `VerifyChain` (which holds the HMAC key). A future unkeyed payload digest committed into the chain (e.g. a plain SHA-256 of `Payload` included in what `EntryHash` covers, or anchored alongside it) would let offline verification also bind `Payload` to the anchored hash, closing this gap.
