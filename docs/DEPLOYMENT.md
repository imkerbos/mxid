...
| `MXID_BOOTSTRAP_ADMIN_PASSWORD` | Initial password for the seeded `admin` account. Migration 000009 seeds that account with a password written in plaintext in the migration itself, in this public repository. Setting this replaces it before the server accepts a request. **Unset, the account still works but owes a password change** and says so loudly in the log on every start. | strong random; a password change is demanded at first sign-in regardless |
...
| `MXID_BOOTSTRAP_ADMIN_PASSWORD` | **release: recommended** | — | Initial password for the seeded `admin` account. Migration 000009 seeds that account with a password written in plaintext in the migration itself, in a public repository. Set this and the account takes your password. Leave it unset and the account **still works**, but owes a password change at first sign-in and logs a loud warning until it is done — so on a fresh install, either set this or sign in and change the password immediately. An earlier release LOCKED the account instead; that left a first deployment, where `admin` is the only account, with nobody able to sign in. A password change is required at first sign-in either way, because this value usually lives in a deployment manifest more people can read than should know it. Helm: `secrets.bootstrapAdminPassword`. |
...
- [ ] `MXID_BOOTSTRAP_ADMIN_PASSWORD` was set before the first start, or the `admin` password was changed immediately after it. Until one of those happens the account accepts a password published in this repository. The forced change at first sign-in is completed, and MFA enrolled.
